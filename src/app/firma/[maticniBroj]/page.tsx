...
interface CompanyProfile {
  ...
  // Social media links (company's own)
...
export default function CompanyMiniWebsite() {
  ...
  const [siteDefaults, setSiteDefaults] = useState<SiteDefaults>({});

  // Fetch site-wide default settings (social links etc.)
  useEffect(() => {
    fetch(`${API_URL}/trpc/siteSettings.getPublic`)
      .then((res) => res.json())
      .then((data) => {
        const settings = data?.result?.data?.json;
        if (settings) setSiteDefaults(settings);
      })
      .catch(() => {});
  }, []);
      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="space-y-6">
            ...
            {/* Social Media Links */}
            {(() => {
              // Use company's own links if registered & has them, otherwise site defaults
              const social = {
                youtube: company.youtubeUrl || siteDefaults.social_youtube || '',
                facebook: company.facebookUrl || siteDefaults.social_facebook || '',
                instagram: company.instagramUrl || siteDefaults.social_instagram || '',
                tiktok: company.tiktokUrl || siteDefaults.social_tiktok || '',
                linkedin: company.linkedinUrl || siteDefaults.social_linkedin || '',
              };
              const hasAnySocial = Object.values(social).some(Boolean);
              ...
              return (
                <section className="bg-white rounded-xl border shadow-sm p-6">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {social.youtube && <SocialLink platform="youtube" href={social.youtube} label="YouTube" />}
                    {social.facebook && <SocialLink platform="facebook" href={social.facebook} label="Facebook" />}
                    {social.instagram && <SocialLink platform="instagram" href={social.instagram} label="Instagram" />}
                    {social.tiktok && <SocialLink platform="tiktok" href={social.tiktok} label="TikTok" />}
                    {social.linkedin && <SocialLink platform="linkedin" href={social.linkedin} label="LinkedIn" />}
                  </div>
                  {!company.registrovan && (
        ...
        <div className="max-w-5xl mx-auto px-4 py-6">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            {(() => {
              ...
              const hasFooterSocial = Object.values(footerSocial).some(Boolean);
              if (!hasFooterSocial) return null;
              return (
                <div className="flex items-center gap-4">
                  {footerSocial.youtube && (
                    <a href={footerSocial.youtube} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-red-600 transition-colors" title="YouTube">
                      <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
                    </a>
                  )}
                  {footerSocial.facebook && (
                    <a href={footerSocial.facebook} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-blue-600 transition-colors" title="Facebook">
                      <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                    </a>
                  )}
                  {footerSocial.instagram && (
                    <a href={footerSocial.instagram} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-pink-600 transition-colors" title="Instagram">
                  ...
                  {footerSocial.tiktok && (
                    <a href={footerSocial.tiktok} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-gray-900 transition-colors" title="TikTok">
